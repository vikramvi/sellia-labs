...
export function getQueriesForDocumentsAround(ref, center, radiusInKm) {
  var geohashesToQuery = geohashQueries(
...
export async function getNearestDocs(collection, center, MAX_DISTANCE = 100) {
  var resultCount = 0,
    totalDistanceKm = 0,
    matchCount = 0,
    matchDistanceKm = 0,
    ...
    queries.map(query => {
      return new Promise(async (resolve, reject) => {
        ...
        snap.forEach(function(doc) {
          ...
          nearest.push({ ...doc.data(), id: doc.id, distance: dist });
        });
        resolve(nearest);
      });
    })
